...
const formFields = [
  ...
  { name: 'Status', label: 'Status', type: 'select' as const, options: ['Pending', 'Allocated', 'Picked', 'Shipped', 'Delivered', 'Cancelled'] },
...
export function SoLinesClient({ soLines: initialData }: { soLines: Record<string, any>[] }) {
  ...
  const stats = useMemo(() => {
    ...
    for (const r of records) {
      ...
      totalShipped += Number(r['Shipped Qty'] ?? 0);
    }
    ...
    return {
      ...
      shippedPct: totalQty > 0 ? ((totalShipped / totalQty) * 100).toFixed(1) : '0',
    };
  }, [records]);
  ...
  const handleCreate = async (values: Record<string, any>) => {
    await createRecord(TABLES.SALES_ORDER_LINES, values);
    setModalMode(null);
  ...
  const handleUpdate = async (values: Record<string, any>) => {
    if (!selected) return;
    await updateRecord(TABLES.SALES_ORDER_LINES, selected.id, values);
    setModalMode(null);
    setSelected(null);
    if (typeof window !== 'undefined') window.location.reload();
  };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    await deleteRecord(TABLES.SALES_ORDER_LINES, selected.id);
  ...
  return (
    <div>
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Sales Order Lines</h1>
          <p className="page-subtitle">SO line items &mdash; quantities, pricing, and shipping progress</p>
        </div>
      ...
      <div className="kpi-grid">
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-value" style={{ color: '#06b6d4' }}>{stats.shippedPct}%</div>
        </div>
      </div>
      ...
      <div className="glass-card">
        <div className="toolbar">
          ...
          <div className="search-bar">
            ...
            <input className="input" placeholder="Search SO lines..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} style={{ width: 260 }} />
          </div>
        </div>
        ...
        {paginated.length === 0 ? (
          <div className="empty-state">
            ...
            {search ? 'No records match your search' : 'No sales order lines yet'}
          </div>
        ) : (
          <>
                  ...
                  {paginated.map((r) => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td style={{ textAlign: 'right', fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>
                          {Number(r['Shipped Qty'] ?? 0).toLocaleString()}
                        </td>
      ...
      {/* ── Create Modal ──────────────────────── */}
      <Modal open={modalMode === 'create'} onClose={() => setModalMode(null)} title="Add SO Line">
        <RecordForm fields={formFields} onSubmit={handleCreate} onCancel={() => setModalMode(null)} submitLabel="Create" />
      </Modal>

      {/* ── Edit Modal ────────────────────────── */}
      <Modal open={modalMode === 'edit'} onClose={() => { setModalMode(null); setSelected(null); }} title="Edit SO Line">
        {selected && (
      ...
      {/* ── Delete Confirmation ────────────────── */}
      <Modal open={modalMode === 'delete'} onClose={() => { setModalMode(null); setSelected(null); }} title="Delete SO Line">
        {selected && (
          <div>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 20 }}>
              Are you sure you want to delete this SO line (Qty: {selected['Quantity'] ?? 'N/A'}, ${Number(selected['Total Price'] ?? 0).toLocaleString()})? This action cannot be undone.
            </p>
